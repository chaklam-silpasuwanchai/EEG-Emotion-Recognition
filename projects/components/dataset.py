...
class DatasetDEAP(torch.utils.data.Dataset): #type: ignore
    '''
    Dataset is a class to handle the DEAP dataset.
    Here we implement 2 labels. VALENCE and AROSAL.
    In case you want both labels, use STIMULI_ALL
    '''
    ...
    def get_data_all(self, stimuli):
        ...
        for filename in self.get_file_list():
            data, labels, groups = self.get_data(filename, stimuli=stimuli)
    ...
    def get_data(self,name:str, stimuli:int = STIMULI_ALL) -> tuple: 
        if(name == 'all'):
    ...
    def get_spec_data(self,name:str, stimuli:int = STIMULI_ALL) -> tuple: 
        if(type(self.data[name]) == type(None) or type(self.labels[name]) == type(None)):
            self._load_data(name)
        data   = self.data[name]
        labels = self.labels[name]
    ...
    def get_all_spec_data(self, stimuli:int = STIMULI_ALL) -> tuple: 
